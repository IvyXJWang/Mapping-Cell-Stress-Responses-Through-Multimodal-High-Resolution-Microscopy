...
for treatment in ["control", "inflammation"]:
    ...
    mito_props[treatment] = all_sheets_filtered[treatment]["mitochondria"].copy()
    mito_props[treatment].loc[:, "ObjectID"] = all_sheets[treatment]["mitochondria"]["ObjectID"]
    mito_props[treatment] = mito_props[treatment].drop("Metadata_CellID", axis=1)

    mito_props_renamed[treatment] = rename_df_columns_with_keyword(mito_props[treatment], rename_dict)

    feature_types = ["Geometric", "Densitometric", "Textural", "Structural"]

    if treatment == "control": # new label
        mito_scaled[treatment], scaler = scale_data_fit(mito_props_renamed[treatment], id_col = "ObjectID")
        feature_groups = {}
        for feature in feature_types:
            cols = mito_scaled[treatment].columns[
                mito_scaled[treatment].columns.str.contains(feature, case=False)
            ]
            
            feature_groups[feature] = cols
        
        mito_props_normalised = apply_group_normalisation(mito_scaled[treatment], feature_groups)
        
        mito_labelled[treatment], _ = add_cluster_label(
        ...
        rf_model, data_scaler, scores, scores_mean, scores_std, feature_importance = (
            random_forest_train_model(
                mito_labelled[treatment], "Subtype", savedir=datadir, scale = False, id_col = "ObjectID"
            )
        )
        
        X_scaled = mito_props_normalised.drop(columns="ObjectID")
        Y = mito_labelled[treatment]["Subtype"]
        
        rf_eval[treatment] = model_eval(X_scaled,Y, rf_model = rf_model)
    
    elif treatment == "inflammation":
        mito_scaled[treatment] = scale_data_transform(mito_props_renamed[treatment], scaler, id_col = "ObjectID")
        mito_props_normalised = apply_group_normalisation(mito_scaled[treatment], feature_groups)

        mito_labelled[treatment], subpopulation_mito_rf, sorted_obj, feature_importance = random_forest_classification(
            rf_model, data_scaler, mito_props_normalised, id_col="ObjectID", scale=False
        )
        mito_labelled[treatment] = mito_labelled[treatment].rename(
    ...
    for i in list(range(1, clusternum + 1)):  # start subpopulation count at 1
        subpopulation_cells[i] = mito_labelled[treatment].loc[
            mito_labelled[treatment]["Subtype"] == i, "ObjectID"
        ].tolist()
    
    subtype_cells[treatment] = subpopulation_cells
    
_, _, cmap_labelling = label_color_mapping_dict(
    mito_labelled["control"]["Subtype"], palette="inferno"
)

#%% feature importance of random forest

a = all_sheets["inflammation"]["mitochondria"]
aa = all_sheets["control"]["mitochondria"]
...
for i in list(range(1, 3 + 1)):  # start subpopulation count at 1
    subpopulation_cells[i] = mito_labelled[treatment].loc[
        mito_labelled[treatment]["Subtype"] == i, constants.CELLID_COL
    ].tolist()
...
all_features = mito_props_renamed["control"].columns
feature = "Geometric_FormFactor"

dataframe_labelled = pd.concat(
    [df[[feature, "ObjectID"]].assign(Treatment=i) 
     for i, (key, df) in enumerate(mito_props_renamed.items(), start=1)],
    ignore_index=True
)
...
#%% compare average mitochondria size in cells - no significant difference in area or circularity 
feature = "Geometric_Area"

df=mito_props_renamed.copy()
for treatment in ["control", "inflammation"]:
    df[treatment]["Metadata_CellID"] = all_sheets[treatment]["mitochondria"]["Metadata_CellID"]

dataframe_labelled = {
    key: df.groupby("Metadata_CellID", as_index=False)[feature].mean()
    for key, df in mito_props_renamed.items()
}
...
subtype_df = mito_props_renamed.copy()
subtype_df["control"]["Subtype"] = mito_labelled["control"]["Subtype"]
subtype_df["inflammation"]["Subtype"] = mito_labelled["inflammation"]["Subtype"]
...
all_features = list(mito_props_renamed["inflammation"].columns)
subtype_df = mito_props_renamed.copy()
subtype_df["control"]["Subtype"] = mito_labelled["control"]["Subtype"]
subtype_df["inflammation"]["Subtype"] = mito_labelled["inflammation"]["Subtype"]
...
measure = "Densitometric_MeanDensitometric_xray"
...
features = [
    ...
    if c not in [comparison_col, condition_col, "ObjectID"]
]
...
g = sns.clustermap(
    effect_mat.T, # .T to plot vertically
    cmap="vlag",
    center=0,
    vmin=-0.3,
    vmax=0.3,
...
g.ax_heatmap.set_yticklabels(new_labels, rotation=90)
...
treatment = "inflammation"
order = [1, 2, 3, 4]
proportions = mito_labelled[treatment]['Subtype'].value_counts(normalize=True)
...
# add cellID back
mito_labelled["inflammation"]["Metadata_CellID"] = all_sheets_filtered["inflammation"]["mitochondria"]["Metadata_CellID"]
plot.stacked_bxplt_count(
    mito_labelled["inflammation"], "Metadata_CellID", "Subtype", 
    label_color_mapping=cmap_labelling, axis_labels = False,
    legend=False
)

plot.stacked_bxplt_count(
    mito_labelled["inflammation"],
    "Metadata_CellID",
    "Subtype",
...
proportions_per_cell = mito_labelled.copy()
proportions_per_cell["control"]["Metadata_CellID"] = all_sheets_filtered["control"]["mitochondria"]["Metadata_CellID"]
proportions_per_cell["inflammation"]["Metadata_CellID"] = all_sheets_filtered["inflammation"]["mitochondria"]["Metadata_CellID"]
...
#%% types of mitochondria cells

mito_cell_types = mito_labelled.copy()
mito_cell_types["control"]["Metadata_CellID"] = all_sheets_filtered["control"]["mitochondria"]["Metadata_CellID"]
mito_cell_types["inflammation"]["Metadata_CellID"] = all_sheets_filtered["inflammation"]["mitochondria"]["Metadata_CellID"]
...
for treatment in ["control", "inflammation"]:
    ...
    comp_id = mito_type[treatment][["Metadata_CellID"]].copy()
    comp_props = mito_type[treatment].drop(columns=["Metadata_CellID"])
    ...
    cluster_df[treatment] = mito_clr.copy()
...
cell_labelled["inflammation"], subpopulation_mito_rf, sorted_obj, feature_importance = random_forest_classification(
...
#%% differences in mito type composition with inflammation
...
df_combined = df_combined[df_combined["Subtype"] == 4]
...
#%% cell mitochondria type proportion
#_, _, cmap_labelling = label_color_mapping_dict(
...
individual_scores_df = silhouette_score_indiv(
    mito_scaled["inflammation"],
    mito_labelled["inflammation"]["Subtype"],
    scale=False,
    cellid_col="ObjectID",
)
silhouette_score_stats_dict = silhouette_score_stats(individual_scores_df, id_col = "ObjectID")
counts = mito_labelled["inflammation"]['Subtype'].value_counts()
...
confidence_filtered =  labelled_inf[labelled_inf["RF_confidence"] > 0.8]
confidence_low =  labelled_inf[labelled_inf["RF_confidence"] <= 0.8]
...
for cell in new_cells:
    ...
    mito_seg_df = all_segmentation["mitochondria"]
    mito_imgs = mito_seg_df[mito_seg_df["Metadata_CellID"] == cell]
    
    outlines =dict(zip('mitochondria_' + mito_imgs['layer'].astype(str), mito_imgs['image']))
...
classification_df = context_classification_mitochondria(
    original_df,
    all_segmentation,
    sheets_df["mitochondria"]["ObjectID"],
    DEBUG = False
)
...
classification_plot = classification_df.merge(mito_labelled["inflammation"][['ObjectID', 'Subtype']],
                                            on='ObjectID',
                                            how='left'
                                            )
